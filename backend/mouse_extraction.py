#!/usr/bin/env python3
"""Mouse feature extraction utilities."""
...
DEFAULT_FIXED_POINTS: Dict[str, Tuple[int, int]] = {
    ...
    "c3": (1483, 1010),
    "c4": (538, 1020),
    "foodhopperBottom": (1020, 751),
    "foodhopperLeft": (677, 756),
    "foodhopperRight": (1298, 725),
    "foodhopperMid": (1015, 562),
}
...
def extract_mouse_features(
    ...
    """Extract features for mouse+nest NPZ. Returns the CSV path."""
    ...
    for key in data.files:
        ...
        if '_obj' not in identifier:
            continue
        frame_idx_str, obj_id_str = identifier.split('_obj', 1)
        try:
    ...
    for frame_idx in sorted(video_segments.keys()):
        ...
        for obj_name, obj_id in [("Mouse", mouse_id), ("Nest", nest_id)]:
        ...
        mouse_pos = (
            all_features[frame_idx].get("Mouse_centroid_x"),
            all_features[frame_idx].get("Mouse_centroid_y"),
        )
        nest_pos = (
            all_features[frame_idx].get("Nest_centroid_x"),
            all_features[frame_idx].get("Nest_centroid_y"),
        )
        all_features[frame_idx]["Mouse_To_Nest_Distance"] = _euclidean_distance(mouse_pos, nest_pos)

        prev_mouse_pos = (
            all_features.get(frame_idx - 1, {}).get("Mouse_centroid_x"),
            all_features.get(frame_idx - 1, {}).get("Mouse_centroid_y"),
        )

        if None not in mouse_pos and None not in prev_mouse_pos:
            dx = float(mouse_pos[0]) - float(prev_mouse_pos[0])
            dy = float(mouse_pos[1]) - float(prev_mouse_pos[1])
            speed = (dx ** 2 + dy ** 2) ** 0.5
            direction_deg = float(np.degrees(np.arctan2(dy, dx)))
            all_features[frame_idx]["Mouse_Speed_px_per_frame"] = speed
            all_features[frame_idx]["Mouse_Direction_deg"] = direction_deg
        else:
            all_features[frame_idx]["Mouse_Speed_px_per_frame"] = None
            all_features[frame_idx]["Mouse_Direction_deg"] = None

        prev_speed = all_features.get(frame_idx - 1, {}).get("Mouse_Speed_px_per_frame")
        curr_speed = all_features[frame_idx].get("Mouse_Speed_px_per_frame")
        all_features[frame_idx]["Mouse_Acceleration_px_per_frame2"] = (
            float(curr_speed - prev_speed) if (curr_speed is not None and prev_speed is not None) else None
        )

        prev_dist = all_features.get(frame_idx - 1, {}).get("Mouse_To_Nest_Distance")
        curr_dist = all_features[frame_idx].get("Mouse_To_Nest_Distance")
        all_features[frame_idx]["Mouse_To_Nest_Dist_Change"] = (
        ...
        for pname, pcoords in fixed_points.items():
            all_features[frame_idx][f"{pname}_Mouse_Distance"] = _euclidean_distance(pcoords, mouse_pos)
