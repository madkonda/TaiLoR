#!/usr/bin/env python3
"""
TaiLOR Video Processing Pipeline for Linux Mint
- Processes videos downloaded from Google Drive
- Extracts frames using ffmpeg (every 10th frame at 30fps)
- Processes frames with SAM2 for mouse/nest segmentation
- Saves results locally on Linux Mint
"""
...
def process_video_with_sam2(video_path, frames_dir, nest_coords, mouse_coords, output_dir):
    ...
    # Import SAM2 (assuming it's installed in the sam2 environment)
    try:
    ...
    # SAM2 configuration - correct paths for Linux Mint
    sam2_checkpoint = "/home/morsestudio/sam2/checkpoints/sam2.1_hiera_large.pt"
    model_cfg = "configs/sam2.1/sam2.1_hiera_l.yaml"  # Relative path from SAM2 package
...
def main():
    parser = argparse.ArgumentParser(description='Process video with SAM2 segmentation on Linux Mint')
    ...
    parser.add_argument('--output_dir', default='/home/morsestudio/sam2/results', help='Output directory')
    ...
    print(f"🚀 Starting video processing pipeline on Linux Mint")
    ...
    print("✅ Video processing completed successfully on Linux Mint!")
    return 0
