...
/**
 * Mac Proxy Server
 * Proxies requests to Linux Mint File Server via Cloudflare Tunnel
 */
...
const LINUX_SERVER_URLS = [
  'https://mintpc.morsestudio.dev',  // Cloudflare tunnel
...
app.post('/api/process-video', async (req, res) => {
  try {
    ...
    const response = await axios.post(`${LINUX_SERVER_URL}/process-video`, {
      video_path,
      nest_x,
      nest_y,
      mouse_x,
...
// Proxy processing status requests
app.get('/api/process-status/:process_id', async (req, res) => {
  try {
    const { process_id } = req.params;
    const response = await axios.get(`${currentLinuxUrl}/process-status/${process_id}`, {
      timeout: 5000
    });
